...
public class PermissionUtils {
    ...
    public static void requestPermission_Fragment(Object object, String permission, int requestCode) {
        ((android.support.v4.app.Fragment) object).requestPermissions(new String[]{permission}, requestCode);
    }
    ...
    public static void checkAndRequestPermission(Context context,Fragment fragment, String permission, int requestCode, PermissionRequestSuccessCallBack callBack) {
        if (checkPermission(context, permission)) {// 用户已授予权限
            callBack.onHasPermission();
        } else {
            requestPermission_Fragment(fragment, permission, requestCode);
        }
    }
